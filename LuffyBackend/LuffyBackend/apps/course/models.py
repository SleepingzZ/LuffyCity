...
class Course(BaseModel):
    ...
    def course_section_list(self):
        ...
        for chapter in course_chapters:
            chapter_sections = chapter.coursesections.all()

            # 获取课时, 并将课时详情追加至 section_detail
            for section in chapter_sections:
                section_detail.append({'id': section.id, 'name': section.name,
                                       'free_trail': section.free_trail, 'section_link': section.section_link})

                # 最多返回四条课时详情
                if len(section_detail) == 4:
...
class Teacher(BaseModel):
    ...
    def role_name(self):
        return self.get_role_display()  # 当前 对象.get_字段名_dispaly()--> 获取 role_choices 对应的字段名
